  ...
  Future resetPassword({
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content:
                Text('Помилка: [error]'.replaceAll('[error]', e.message!))),
      );
      return null;
    }
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Посилання на зміну паролю надіслали на пошту')),
    );
  }
    ...
    phoneAuthManager.addListener(() {
      ...
      if (phoneAuthManager.triggerOnCodeSent) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Помилка: [error]'.replaceAll('[error]', e.message!)),
        ));
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      return userCredential == null
          ? null
          : BeautyBookingFirebaseUser.fromUserCredential(userCredential);
    } on FirebaseAuthException catch (e) {
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
